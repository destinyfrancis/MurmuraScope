...
class TestScorePost:
    def test_chronological_pure_recency(self):
        ...
        post_same_round = {
            "id": "1", "agent_id": 1, "round_number": 5,
            "content": "test", "sentiment": "neutral", "trust_score": 0.5
        }
        post_older = {
            "id": "2", "agent_id": 2, "round_number": 1,
            "content": "test", "sentiment": "neutral", "trust_score": 0.5
        }
    ...
    def test_engagement_first_longer_content_higher(self):
        ...
        post_long = {
            "id": "1", "agent_id": 1, "round_number": 3,
            "content": "a" * 200, "sentiment": "neutral", "trust_score": 0.5
        }
        post_short = {
            "id": "2", "agent_id": 2, "round_number": 3,
            "content": "hi", "sentiment": "neutral", "trust_score": 0.5
        }
    ...
    def test_echo_chamber_high_trust_ranks_higher(self):
        ...
        post_high_trust = {
            "id": "1", "agent_id": 1, "round_number": 3,
            "content": "test", "sentiment": "positive", "trust_score": 0.9
        }
        post_low_trust = {
            "id": "2", "agent_id": 2, "round_number": 3,
            "content": "test", "sentiment": "positive", "trust_score": 0.1
        }
    ...
    def test_score_in_valid_range(self):
        ...
        for algo, weights in ALGORITHM_WEIGHTS.items():
            post = {
                "id": "1", "agent_id": 1, "round_number": 5,
                "content": "test post", "sentiment": "positive", "trust_score": 0.5
            }
...
class TestRankFeed:
    def _make_rows(self, n: int) -> list:
        ...
        return [
            (i, i, 5, f"Post content {i}", "neutral", "economy", 0.5)
    ...
    async def test_returns_empty_with_no_posts(self):
        ...
        result = await engine.rank_feed(
            "s", 1, 0.5, 5, FeedAlgorithm.ENGAGEMENT_FIRST, mock_db
        )
        assert result == []
    ...
    async def test_chronological_returns_most_recent_first(self):
        ...
        result = await engine.rank_feed(
            "s", 1, 0.5, 10, FeedAlgorithm.CHRONOLOGICAL, mock_db
        )
        assert len(result) == 2
...
class TestComputeBubbleIndex:
    ...
    async def test_single_author_max_source_concentration(self):
        ...
        engine = FeedRankingEngine()
        feed = [
            {"agent_id": 42, "sentiment": "positive", "round_number": 3}
            for _ in range(10)
        ]
        idx = await engine.compute_bubble_index(1, 0.5, feed, 3)
    ...
    async def test_multiple_authors_lower_concentration(self):
        ...
        engine = FeedRankingEngine()
        feed = [
            {"agent_id": i, "sentiment": "neutral", "round_number": 3}
            for i in range(10)
        ]
        idx = await engine.compute_bubble_index(1, 0.5, feed, 3)
...
class TestComputeBubbleReport:
    ...
    async def test_empty_indices_zero_report(self):
        ...
        report = await engine.compute_bubble_report(
            "s", 5, FeedAlgorithm.ENGAGEMENT_FIRST, []
        )
    ...
    async def test_pct_in_bubble_correct(self):
        ...
        report = await engine.compute_bubble_report(
            "s", 3, FeedAlgorithm.ENGAGEMENT_FIRST, indices
        )
    ...
    async def test_algorithm_name_preserved(self):
        engine = FeedRankingEngine()
        report = await engine.compute_bubble_report(
            "s", 3, FeedAlgorithm.ECHO_CHAMBER, [
                FilterBubbleIndex(1, 3, 0.5, 0.3, 0.4, 0.6)
            ]
        )
...
class TestFilterBubbleIndexModel:
    def test_frozen(self):
        idx = FilterBubbleIndex(
            agent_id=1, round_number=3,
            exposure_diversity=0.5, stance_divergence=0.3,
            source_concentration=0.4, bubble_score=0.6,
        )
    ...
    def test_fields(self):
        idx = FilterBubbleIndex(
            agent_id=99, round_number=7,
            exposure_diversity=0.8, stance_divergence=0.2,
            source_concentration=0.1, bubble_score=0.15,
        )
...
class TestFilterBubbleReportModel:
    def test_frozen(self):
        r = FilterBubbleReport(
            session_id="s", round_number=3,
            avg_bubble_score=0.4, median_bubble_score=0.38,
            pct_in_bubble=0.1, algorithm_name="chronological",
