...
def run_audit():
    ...
    macro_state = MacroState(
        ...
        mortgage_cap=0.7,
        stamp_duty_rates={"ad_valorem_scale_2": 0.0375},
        gdp_growth=0.032,
        cpi_yoy=0.021,
        hsi_level=16800.0,
    ...
    for rule_name, fn in rules_demo.items():
        ...
        report["demographics"].append(
            {"rule": rule_name, "failed_count": len(failed), "failed_examples": [a.__dict__ for a in failed[:1]]}
        )

    main_student = sum(1 for a in student_ages if 15 <= a <= 25)
    report["demographics"].append(
        {"rule": "student age (15-25 main)", "failed_count": len(student_ages) - main_student, "failed_examples": []}
    )
    ...
    rules_occ = {
        ...
        "Working adult (22-64) non-zero income": lambda a: (
            not (22 <= a.age <= 64 and a.occupation not in ("學生", "退休") and a.monthly_income == 0)
        ),  # Exception for unemployed implicitly handled if income == 0 means unemployed, but user requirement says NO ZERO INCOME unless unemployed. Our factory uses 0 for unemployed. Let's see if the factory has unemployed. Factory rule: "working adults with zero income receive floor" => so there should be NO zero income working adults!
    }
    for rule_name, fn in rules_occ.items():
        failed = [a for a in agents if not fn(a)]
        report["occupation_income"].append(
            {"rule": rule_name, "failed_count": len(failed), "failed_examples": [a.__dict__ for a in failed[:1]]}
        )

    # 3. Housing / Financial
    rules_housing = {
        "Private + 0 income + savings < 200k = Impossible": lambda a: (
            not (a.housing_type == "私人住宅" and a.monthly_income == 0 and a.savings < 200000)
        ),
        "PRH income <= threshold": lambda a: (
            not (a.housing_type == "公屋" and a.monthly_income > 28180)
        ),  # 13660 for single, but we don't know family size. Let's use 4-person upper bound 28180.
        "Temp housing high income (>40k) in-viable": lambda a: (
            not (a.housing_type == "臨時／其他" and a.monthly_income > 40000)
        ),
        "Subsidized sale extremely low savings (<50k)": lambda a: (
            not (a.housing_type == "資助出售房屋" and a.savings < 50000 and a.age < 30)
        ),  # Must have paid downpayment or inherited
    }
    for rule_name, fn in rules_housing.items():
        failed = [a for a in agents if not fn(a)]
        report["housing_finance"].append(
            {"rule": rule_name, "failed_count": len(failed), "failed_examples": [a.__dict__ for a in failed[:1]]}
        )
    ...
    for rule_name, fn in rules_marital.items():
        if rule_name in ["Age < 22 married should be < 5%", "Widowed mostly >= 60"]:
            continue
        failed = [a for a in agents if not fn(a)]
        report["age_marital"].append(
            {"rule": rule_name, "failed_count": len(failed), "failed_examples": [a.__dict__ for a in failed[:1]]}
        )
    ...
    report["age_marital"].append(
        {
            "rule": "Age < 22 married should be < 5%",
            "failed_count": len(u22_married) if u22_married_pct >= 0.05 else 0,
            "failed_examples": [a.__dict__ for a in u22_married[:1]],
        }
    )
    ...
    report["age_marital"].append(
        {
            "rule": "Widowed mostly >= 60",
            "failed_count": len(young_widows) if widow_pct > 0.3 else 0,
            "failed_examples": [a.__dict__ for a in young_widows[:1]],
        }
    )

    # 5. Edu / Occ
    edu_occ_failed1 = [
        a for a in agents if a.occupation in ("專業人員", "經理及行政人員") and a.education_level == "小學或以下"
    ]
    report["education_occupation"].append(
        {
            "rule": "Manager/Professional + Primary school (<2%)",
            "failed_count": len(edu_occ_failed1),
            "failed_examples": [a.__dict__ for a in edu_occ_failed1[:1]],
        }
    )
    ...
    report["education_occupation"].append(
        {
            "rule": "Elementary + Degree (<5%)",
            "failed_count": len(edu_occ_failed2),
            "failed_examples": [a.__dict__ for a in edu_occ_failed2[:1]],
        }
    )
    ...
    for rule, fails in persona_failures.items():
        report["persona_text"].append(
            {"rule": rule, "failed_count": len(fails), "failed_examples": [a.__dict__ for a in fails[:1]]}
        )
    ...
    for d, incomes in districts.items():
        report["regional_income"][d] = {
            "median": float(np.median(incomes)) if incomes else 0,
            "std": float(np.std(incomes)) if incomes else 0,
            "count": len(incomes),
        }
    ...
    buy_prop_fails2 = [
        a for a in agents if a.monthly_income == 0 and a.savings < 1500000 and is_eligible_buy_property(a, macro_state)
    ]
    ...
    report["decision_rules"].append(
        {
            "rule": "PRH should not buy_property",
            "failed_count": len(buy_prop_fails),
            "failed_examples": [a.__dict__ for a in buy_prop_fails[:1]],
        }
    )
    report["decision_rules"].append(
        {
            "rule": "Zero income should not buy property",
            "failed_count": len(buy_prop_fails2),
            "failed_examples": [a.__dict__ for a in buy_prop_fails2[:1]],
        }
    )
    report["decision_rules"].append(
        {
            "rule": "Retiree should not change job",
            "failed_count": len(job_fails),
            "failed_examples": [a.__dict__ for a in job_fails[:1]],
        }
    )
    report["decision_rules"].append(
        {
            "rule": "Student should not invest property (invest in general here)",
            "failed_count": len(invest_fails),
            "failed_examples": [a.__dict__ for a in invest_fails[:1]],
        }
    )
